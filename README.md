...
1. `quality / VP — fmt, oxlint, TypeScript`（`bun run check` — [`package.json`](./package.json) の `check` が **Bun で `vp` バイナリを直接実行**し、`vite.config.ts` を解決する。素の `vp` / `bunx vp` は Node 起動になり TS 設定で落ちる）
